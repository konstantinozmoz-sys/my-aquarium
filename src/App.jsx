...
const IDEAL_PARAMS = {
  ...
  kh: { min: 7.5, max: 11.0, unit: 'dKH', name: 'Щелочность (KH)', desc: 'Буферная емкость воды. Главный параметр для роста скелетов жестких кораллов.' },
  ...
  no3: { min: 2, max: 15, unit: 'ppm', name: 'Нитраты (NO3)', desc: 'Показатель органики. В малых дозах — еда для кораллов.' },
  po4: { min: 0.03, max: 0.1, unit: 'ppm', name: 'Фосфаты (PO4)', desc: 'Продукт жизнедеятельности. Избыток блокирует рост кораллов.' },
          ...
          {aquariums.map(aq => (
            <div key={aq.id} className="premium-card">
              ...
              <div className="mt-6 flex justify-between px-2 italic font-black opacity-30 items-center">
                 <span className="text-[8px] text-slate-700 uppercase tracking-widest italic font-black">Био: {coralsCount} ед.</span>
                 <span className="text-[8px] text-slate-800 uppercase italic font-black">Подмена: {new Date(aq.lastWaterChange).toLocaleDateString()}</span>
              </div>
  ...
  const Lab = () => {
    ...
    return (
      <div className="view-container animate-fadeIn font-black italic leading-none font-black italic font-black">
        ...
        <div className="mt-8 space-y-3 leading-none italic font-black pb-32">
          {Object.entries(IDEAL_PARAMS).map(([k, v]) => (
            <div key={k} className="premium-card !p-4 flex justify-between items-center shadow-lg bg-slate-900/40 border-none leading-none italic font-black">
              ...
              <input type="number" step="0.1" value={local[k]} onChange={e => setLocal({...local, [k]: e.target.value})} className="param-input italic shadow-inner bg-[#020617] font-black" />
            </div>
          ))}
        ...
        <div className="space-y-4 font-black italic leading-none italic font-black">
           {list.length === 0 ? (
               ...
               <div key={item.id} className="premium-card !p-5 flex justify-between items-center group leading-none italic shadow-xl bg-slate-900/40 border-none italic font-black font-black font-black italic font-black italic font-black">
                  ...
                  }} className="text-slate-800 hover:text-red-500 p-2 opacity-30 group-hover:opacity-100 transition-opacity italic leading-none font-black italic font-black italic font-black leading-none font-black italic font-black shadow-inner"><Trash2 size={16}/></button>
               </div>
           ))}
        </div>
      </div>
  ...
  const Doctor = () => {
    ...
    const handleDoc = async (e) => {
      ...
      try {
        ...
        const txt = await callVision(base64, f.type, "Analyze photo for coral/fish diseases. Briefly in Russian.");
        setRes(txt);
  ...
  return (
    <div className="app-shell italic font-black leading-none italic font-black italic font-black leading-none italic font-black italic font-black leading-none italic font-black italic font-black">
      <style>{`
        ...
        .param-input { width: 6rem; background: #020617 !important; border: 1px solid #1e293b; border-radius: 1rem; color: white !important; text-align: center; font-family: monospace; font-size: 1.1rem; padding: 0.6rem; outline: none; box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); }
        ...
        .profile-input { width: 100%; background: #020617 !important; border: 1px solid #1e293b; border-radius: 1.5rem; padding: 1.5rem; color: white !important; outline: none; font-size: 1rem; font-style: italic; shadow-inner; font-weight: 700; transition: 0.3s; text-align: center; box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); }
        .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.92); backdrop-filter: blur(35px); display: flex; align-items: center; justify-content: center; p: 2rem; z-index: 5000; }
        ...
        .auth-input { width: 100%; background: #020617 !important; border: 1px solid #1e293b; border-radius: 1.5rem; padding: 1.5rem; color: white !important; text-align: center; font-weight: 900; font-size: 1.1rem; outline: none; transition: 0.3s; box-shadow: inset 0 2px 4px rgba(0,0,0,0.5); }
