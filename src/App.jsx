  ...
  const DoctorView = () => {
    ...
    const testApi = async () => {
        try {
            ...
            const data = await response.json();
            if (data.models) alert("✅ Neural Core Authorized.");
    ...
    const identifyDisease = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GOOGLE_API_KEY}`, {
                ...
                body: JSON.stringify({
                    contents: [{ parts: [
                        { text: "Analyze this aquarium photo. Identify health issues (RTN/STN, parasites, ich). Reply in Russian. Be clinical but concise. Focus on treatment." },
                        { inline_data: { mime_type: file.type, data: base64Data } }
                    ]}]
                })
            });
